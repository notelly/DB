...
FROM employees
WHERE salary >= 7000 --BETEEN 사용도 가능
AND salary <= 12000
...
SELECT employee_id, last_name, hire_date, SUBSTR(email, 1,3) AS EMAIL,LENGTH(email) AS EMAILLENGTH --이상하게 풀었음
FROM employees
...
--TRUNC(MONTHS_BETWEEN(sysdate, hire_date)) AS TotalWorked LAST_DAY(hire_date)+1 AS Firstpay
FROM employees
ORDER BY hire_date ;
...
SELECT employee_id, last_name, TO_CHAR(hire_date, 'DD/MM/rrrr DAY') AS hiredate, TO_CHAR(NVL(commission_pct,0)*salary, ''$99,999.99'') AS extrapay
FROM employees
...
SELECT employee_id, last_name, salary*(1+NVL(commission_pct,0))*12 AS year_salary, TO_CHAR(hire_date, 'RRRR') AS hire_year,
        ...
        --NVL2 을 쓰는 방법도 있다.
FROM employees
ORDER BY year_salary ASC, department_id ASC;
...
SELECT employee_id, last_name, department_id,
        CASE country_id WHEN 'CA' THEN 'CANADA'
                           WHEN 'UK' THEN 'UK' --사실 컨트리 말고 걍 부서번호 하고 조인 안해도 됨
        ELSE 'USA'
        END AS country
